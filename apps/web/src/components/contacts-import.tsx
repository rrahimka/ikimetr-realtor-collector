'use client';import { useState } from 'react';import { useRouter } from 'next/navigation';import { t, tCsvError, type Lang } from '../lib/i18n';
...
return <div className="panel"><h2>{t(lang,'import.title')}</h2><form onSubmit={async e=>{e.preventDefault();setBusy(true);setError('');const f=new FormData(e.currentTarget);const file=f.get('file');if(!(file instanceof File)||!file.name){setError(t(lang,'csvError.fileRequired'));setBusy(false);return;}const body=new FormData();body.append('file',file);const r=await fetch('/api/import/contacts',{method:'POST',headers:{'x-csrf-token':csrf()},body});setBusy(false);if(r.ok){setReport(await r.json() as Report);router.refresh();}else{const message=((await r.json().catch(()=>({error:'Request failed'}))) as {error?:string}).error??'Request failed';setError(tCsvError(lang,message));}}}>
...
<a href="/api/import/contacts">{t(lang,'import.downloadTemplate')}</a><span className="muted">{t(lang,'import.templateHint')}</span></form>
...
{report&&report.errors.length>0&&<ul className="errors">{report.errors.map((er,i)=><li key={i}>{t(lang,'import.rejected')} #{er.line}: {tCsvError(lang,er.reason)}</li>)}</ul>}
</div>}
