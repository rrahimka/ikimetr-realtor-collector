...
1. В файле `.env` укажите параметры Telegram (если требуется API):
   ...
   TELEGRAM_API_HASH=...
   ```
2. Коллектор автоматически анализирует открытые сообщения и комментарии в подключенных группах. Личные переписки (DMs) и скрытые чаты программно исключены политикой коннектора и валидацией данных (Private DMs are programmatically excluded by connector policy and validation).
